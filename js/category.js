...
async function showCategory() {
    ...
    for (let i = 0; i < result.categories.length; i++) {
        ...
        <div class="meal-img">
            <div><img   class='w-100' src=${result.categories[i].strCategoryThumb} alt=""></div>
            <div  onclick="showDetailsOfCategory('${result.categories[i].strCategory}')"  class="meal-layer text-center p-1">
                <h2>${result.categories[i].strCategory}</h2>
                <p>${result.categories[i].strCategoryDescription.slice(0,120)}</p>
            </div>
        </div>
        </div>
        `;
    }
    categoryDiv.html(carotona);
}
showCategory()
...
//show datails of category

async function showDetailsOfCategory(x) {
    console.log(x);
