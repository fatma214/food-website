...
async function showAreas() {
    ...
    for (let i = 0; i < result.meals.length; i++){
        console.log(`${result.meals[i].strArea}`);
        carotona +=` 
        <div class="col-md-3" onclick="showMeals('${result.meals[i].strArea}')" id="area">
...
async function showMeals(x) {
    console.log(x);
    var data = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${x}`);
    var result = await data.json();
    console.log(result.meals);
